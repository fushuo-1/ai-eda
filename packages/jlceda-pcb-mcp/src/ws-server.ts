...
export class MCPServerWebSocket {
   ...
   */
  static async create(port: number = 8765, timeout: number = DEFAULT_REQUEST_TIMEOUT): Promise<MCPServerWebSocket> {
   ...
   */
  private constructor(port: number = 8765, timeout: number = DEFAULT_REQUEST_TIMEOUT) {
  ...
  private handleMessage(clientId: string, data: Buffer): void {
    try {
      const message: WebSocketRequest | WebSocketResponse = JSON.parse(data.toString());
   ...
   */
  private sendPong(clientId: string, pingId: string, params?: any): void {
    ...
    const pongResponse: WebSocketResponse = {
      id: pingId,
      result: {
        status: 'ok',
   ...
   */
  private handleShutdown(clientId: string, requestId: string, _params?: any): void {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!client) {
        reject(new MCPError(
          ...
          'Extension may be reconnecting. Please try again.'
        ));
        return;
      }
      ...
      const timer = setTimeout(() => {
        ...
        reject(new MCPError(
          ...
          `Request timeout (${requestTimeout}ms). Extension may be busy or disconnected.`,
          'Check if JLCEDA Pro is responsive and try again.'
        ));
      }, requestTimeout);
      ...
      client.send(JSON.stringify(request), (error: Error) => {
        if (error) {
          this.requestManager.delete(id);
          this.pendingRequests.delete(id);
          reject(error);
