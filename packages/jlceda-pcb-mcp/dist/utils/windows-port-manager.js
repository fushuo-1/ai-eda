...
export class WindowsPortManager {
     ...
     */
    async checkAndClearPort(port, currentPid) {
        ...
        console.error(`[PortManager] Terminating process ${info.processName} ` +
            `(PID: ${info.pid}) using port ${port}...`);
        const result = await this.killProcess(info.pid);
        if (!result.success) {
            console.error(`[PortManager] Failed to terminate process ${info.pid}: ${result.error}`);
            return false;
        }
     ...
     */
    async detectPortOccupancy(port) {
        try {
...
//# sourceMappingURL=windows-port-manager.js.map