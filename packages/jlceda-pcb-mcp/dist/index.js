...
// ====================================================================
server.registerTool('sch_get_netlist', {
    title: '获取原理图网表',
...
返回:
  ...
  - 统计信息`,
    ...
    components.slice(0, 20).forEach((c, i) => {
    ...
    nets.slice(0, 15).forEach((net, i) => {
        const pinStr = net.pins.slice(0, 5).map((p) => `${p.designator}-${p.pin}`).join(', ');
