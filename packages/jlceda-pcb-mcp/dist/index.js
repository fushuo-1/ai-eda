...
import { formatBoardOutlineMarkdown, formatPCBComponentsMarkdown, formatLayerCountMarkdown, formatComponentRelativePositionMarkdown, formatNearbyComponentsMarkdown, formatComponentBoundingBoxMarkdown, formatCollisionCheckMarkdown, formatOverlapCheckMarkdown, formatSpacingCheckMarkdown, truncateResponse, } from './services/formatter.js';
// Constants
 ...
 */
async function executeTool(methodName, params, handler) {
    try {
...
// Schematic Tools
// ====================================================================
server.registerTool('sch_get_all_components', {
    title: '获取原理图所有元器件',
...
server.registerTool('sch_get_component_by_designator', {
    title: '按位号获取原理图元器件',
...
server.registerTool('sch_get_component_pins', {
    title: '获取原理图元器件引脚',
...
// ====================================================================
server.registerTool('sch_get_bom', {
    title: '获取原理图 BOM',
...
返回:
  ...
  - 统计信息`,
    ...
    if (components.length > 0 && grouped.length === 0) {
        ...
        components.slice(0, 30).forEach((c, i) => {
...
server.registerTool('sch_get_netlist_file', {
...
返回 (JSON数据):
  ...
  - stats: 统计信息`,
    ...
    components.slice(0, 15).forEach((c, i) => {
    ...
    nets.slice(0, 10).forEach((net, i) => {
        const pins = net.pins.slice(0, 4).map((p) => `${p.designator}.${p.pin}`).join(', ');
...
// PCB Tools
// ====================================================================
server.registerTool('pcb_get_board_outline_size', {
    title: '获取 PCB 板框尺寸',
...
server.registerTool('pcb_get_board_outline_position', {
    title: '获取 PCB 板框位置',
...
返回: 边界框、中心点和所有顶点`,
    ...
    outline.vertices.forEach((v, idx) => {
...
server.registerTool('pcb_get_all_components', {
    title: '获取 PCB 所有元器件',
...
server.registerTool('pcb_get_top_layer_components', {
    title: '获取 PCB 顶层元器件',
...
server.registerTool('pcb_get_bottom_layer_components', {
    title: '获取 PCB 底层元器件',
...
server.registerTool('pcb_find_component_by_designator', {
    title: '按位号查找 PCB 元器件',
...
server.registerTool('pcb_get_component_pads', {
    title: '获取 PCB 元器件焊盘',
...
server.registerTool('pcb_set_component_transform', {
...
server.registerTool('pcb_generate_netlist_map', {
    title: '生成 PCB 网表映射',
...
server.registerTool('pcb_get_layer_count', {
    title: '获取 PCB 层数',
...
server.registerTool('pcb_calculate_component_relative_position', {
    title: '计算PCB元器件相对位置',
...
server.registerTool('pcb_find_nearby_components', {
    title: '查找邻近器件',
...
server.registerTool('pcb_calculate_component_bounding_box', {
    title: '计算PCB元器件封装边界盒',
...
server.registerTool('pcb_check_component_collision', {
...
server.registerTool('pcb_check_component_overlap', {
    title: '检测器件重叠',
...
server.registerTool('pcb_check_component_spacing', {
    title: '检测器件间距',
